export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexTurnStateHint: "Collect, verify, and renew Turn State separately for each account and requested model; clients do not enter it manually. The same account and model may replay it across IPs. One hour is only the local retention ceiling, not an upstream validity guarantee.",
        codexTurnStateAutoTitle: "Automatically manage Turn State",
        codexTurnStateAutoHint: "Off by default. A candidate replaces the old value only after the real response.created and response.completed models both match and replay succeeds on the same proxy session and the daily proxy. State length is diagnostic only. A 429 stops IP rotation and applies Retry-After backoff. Idle accounts are not kept alive. Disabling retains verified values but stops automatic collection, injection, and probes.",
        codexTurnStateDefaultModel: "Default probe model",
        codexTurnStateDefaultModelHint: "Used only when no request model is specified. Acquisition, renewal, and proxy replay use the requested-model slot that owns the state; states cannot be shared across models. codex-auto-review is a valid, independent model name. Enter a full model name, or leave blank to restore gpt-5.5.",
        ...
        codexTurnStateModelsHint: "Comma-separated, case-insensitive and deduplicated. A trailing * matches a prefix. Either the client model or mapped upstream model may match. Up to 1024 bytes after normalization.",
