export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexTurnStateTitle: "Codex Turn State (automatic / experimental)",
        codexTurnStateHint: "Collect, verify, and renew Turn State separately for each account and requested model; clients do not enter it manually. The configured model scope gates automatic collection, injection, and manual collection; account schedulability, proxy status, and account concurrency do not. One hour is only the local retention ceiling, not an upstream validity guarantee.",
        codexTurnStateAutoTitle: "Automatically manage Turn State",
        codexTurnStateAutoHint: "Off by default. Only models in the configured scope are collected and injected; an empty scope matches all models. A candidate enters real usage verification only after the response.created and response.completed models match and replay succeeds on its collection route. State length is diagnostic only, and a 429 applies Retry-After backoff. Disabling retains verified values and stops automatic collection, injection, and probes; manual collection remains available.",
        codexTurnStateProxy: "Dedicated dynamic proxy",
        codexTurnStateProxyHint: "Optional. Prefer the selected proxy for Turn State collection and renewal. A 1024Proxy dynamic template creates fresh sessions; other proxies are used as-is. An unavailable record falls back to the account proxy or direct transport instead of blocking collection. Proxy passwords are never shown here.",
        codexTurnStateDefaultModel: "Default probe model",
        codexTurnStateDefaultModelHint: "Used only when a request does not specify a model. Acquisition, renewal, and replay still use the requested-model slot that owns the state; states cannot be shared across models. When the model scope is non-empty, this model must match it. codex-auto-review is a valid, independent model name. Enter a full model name, or leave blank to restore gpt-5.5.",
        codexTurnStateInvalidDefaultModel: "Enter one model name, up to 128 characters, using letters, numbers or - _ . : /. Spaces and wildcards are not supported.",
        codexTurnStateModels: "Model scope",
        codexTurnStateModelsPlaceholder: "e.g. gpt-5.5,gpt-5.5-*; empty matches all",
        codexTurnStateModelsHint: "Automatic collection, injection, and manual collection only handle matching models. Comma-separated, case-insensitive and deduplicated; a trailing * matches a prefix. Either the client model or mapped upstream model may match. Up to 1024 bytes after normalization.",
        codexTurnStateInvalidModels: "Invalid model scope: use exact names or a trailing * prefix, up to 1024 bytes, without spaces or quotes.",
