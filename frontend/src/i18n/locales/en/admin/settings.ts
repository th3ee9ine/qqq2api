export default {
    settings: {
      ...
      description: 'Manage site, feature, security, and gateway settings',
      tabs: {
        general: 'General',
      ...
      features: {
        channelMonitor: {
          ...
          configureLink: 'Configure channel monitors',
      ...
      security: {
        ...
        stepUpHint: 'When enabled, sensitive operations (account/proxy export and promoting admins) require a recent TOTP verification (valid for 15 minutes). Your own account must have 2FA enabled before turning this on; turning it off also requires step-up verification.',
      ...
      panelRateLimit: {
        ...
        proxySafeNote: 'Authenticated endpoints are counted for the administrator session, independent of source IP. Public endpoints are counted per real client IP, while loopback/private proxy hops are skipped.',
        enabled: 'Enable panel rate limiting',
        enabledHint: 'Limits administrator panel endpoints. Requests over the threshold get HTTP 429 and recover automatically when the window resets.',
        userRpm: 'Administrator requests',
        userRpmHint: 'Total panel API requests allowed for the administrator per minute. 0 = unlimited.',
        heavyRpm: 'Administrator heavy queries',
        heavyRpmHint: 'Usage/dashboard aggregation queries allowed for the administrator per minute. 0 = unlimited.',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: 'When an account\'s current OpenAI Codex or Anthropic usage window reaches this percent, Sub2API temporarily removes it from scheduling until the window resets. Use 100 to disable.',
        ...
        accountSchedulingThresholdsRangeHint: 'Integer 1–100 (percent). OpenAI and Anthropic only.'
      },
      ...
      gatewayForwarding: {
        ...
        fingerprintUnificationHint: 'Unify X-Stainless-* headers across client requests sharing the same OAuth account. Disabling passes through each client\'s original headers.',
        ...
        accountLocalDeviceIdentityHint: 'When enabled, OpenAI Codex requests prefer the account device session Originator, User-Agent, and Version together. The explicit local Version takes priority; when absent, it is derived from the leading UA version, which is kept in sync. Valid local versions are not raised by the global version floor. A missing, invalid, or mismatched local identity falls back as a whole to the global settings and baseline identity below.',
        openaiCodexIdentityTitle: 'GPT/Codex Upstream Identity Headers',
        openaiCodexIdentityHint: 'Configure the global client identity used for GPT/Codex calls to OpenAI upstream. The displayed baseline defaults describe only the result when all three global fields are empty; with a mixed configuration, Originator and User-Agent are resolved as a paired identity. When account device-session identity is enabled and valid, the account Originator, User-Agent, and Version all take precedence.',
        codexTurnStateTitle: "Codex Turn State (global / experimental)",
        codexTurnStateHint: "Disabled by default. Overrides X-Codex-Turn-State for matching Codex Responses / Compact / WebSocket requests globally, without per-account settings. Disabling keeps the saved token and stops only the override; models and reasoning are unchanged.",
        codexTurnStateNotice: "Public-envelope inspection only; signatures, ownership, and upstream efficacy are not verified. The 10-block baseline and 1-hour reference TTL are community heuristics. Expired, future, larger envelopes and unknown formats are warnings only, not injection gates. This does not guarantee quality or prevent Overload, and the separate automatic lifecycle switch controls collection and renewal. With automatic mode enabled, a 312 state revokes the current value early and overrides the warning-only policy.",
        codexTurnStateAutoTitle: "Automatically collect, generate and renew",
        codexTurnStateAutoHint: "Off by default, independent of the manual global override. Collects real response tokens per account. Matching requests with a missing token or one near the reference 1-hour lifetime trigger an asynchronous Responses probe to obtain an upstream-issued token. Probes may consume upstream quota. Renewal starts 10 minutes early. A 312 state (11 cipher blocks) immediately revokes the old value and triggers a probe for a new 292 state (10 blocks), bypassing the regular 5-minute cooldown once. Failed recovery retains backoff. Revoked manual and native states cannot override the replacement; idle accounts are not kept alive. Disabling retains collected values but stops automatic collection, injection and subsequent probes.",
        codexTurnStateToken: "Turn State token",
        codexTurnStateTokenPlaceholder: "Paste an upstream X-Codex-Turn-State value",
        codexTurnStateTokenConfiguredPlaceholder: "Configured; leave blank to preserve (never echoed)",
        codexTurnStateTokenHint: "Printable ASCII only, up to 4096 characters. Paste to replace or leave blank to preserve. Click Clear and save to remove the stored value.",
        codexTurnStateClear: "Clear",
        codexTurnStateModels: "Model scope",
        codexTurnStateModelsPlaceholder: "e.g. gpt-5.5,gpt-5.5-*; empty matches all",
        codexTurnStateModelsHint: "Comma-separated, case-insensitive and deduplicated; a trailing * matches a prefix. Either the client or mapped upstream model may match; both unknown also qualifies. Maximum 1024 normalized bytes.",
        codexTurnStateStatusLabel: "Manual global configuration diagnostics",
        codexTurnStatePreview: "Local preview of unsaved token",
        codexTurnStateVerdict: "Envelope verdict: {verdict}",
        codexTurnStateBlocks: "Fernet cipher blocks: {blocks} (reference baseline ≤ 10)",
        codexTurnStateSetAt: "Last set: {value}",
        codexTurnStateIssuedAt: "Public issue time: {value}",
        codexTurnStateExpiresIn: "Reference 1-hour TTL remaining: {duration} ({value})",
        codexTurnStateExpiredAgo: "Reference TTL exceeded by {duration} ({value}); warning only, still injected",
        codexTurnStateInjectionActive: "After saving this form, matching requests will use the configured value; this does not mean upstream has accepted it.",
        codexTurnStateInjectionInactive: "After saving this form, no configured value is injected by the manual global override: disabled, not configured, or unsafe as an HTTP header. Automatic account values use the separate switch above.",
        codexTurnStateInvalidToken: "Turn State must contain printable ASCII only and be at most 4096 characters.",
        codexTurnStateInvalidModels: "Invalid model scope: exact values or trailing * prefixes only; 1024 normalized bytes total; no whitespace or quotes.",
        codexTurnStateStatus: {"disabled": "Disabled", "missing": "Token not configured", "invalid": "Unsafe HTTP header value", "unknown": "Unknown envelope (warning only)", "ready": "Within envelope baseline (not efficacy-verified)", "future": "Future issue time (warning only)", "expired": "Past reference TTL (warning only)", "suspect": "Cipher blocks above reference baseline (warning only)"},
        openaiCodexOriginator: 'Originator',
        openaiCodexOriginatorPlaceholder: 'Baseline applies when all three global fields are empty',
        openaiCodexOriginatorHint: 'Originator for Responses/WS inference and credential/control-plane endpoints such as auth, quota, and input_tokens. Selecting a preset fills its paired UA; manual values remain supported. A non-empty value also becomes the leading UA client name.',
        openaiCodexUserAgent: 'User-Agent',
        openaiCodexUserAgentPlaceholder: 'Baseline applies when all three global fields are empty',
        openaiCodexUserAgentHint: 'The complete UA must begin with client/version. Presets are editable templates, not fingerprints measured on this machine; adjust the platform as needed. Choosing a template also sets Originator; choosing Version updates the leading UA engine version. Desktop trailer app builds remain independent.',
        openaiCodexClientVersion: 'Version',
        openaiCodexClientVersionPlaceholder: 'Baseline applies when all three global fields are empty',
        openaiCodexClientVersionHint: 'Selecting history or entering a version switches to pinned mode; save to use that exact stable version, including versions below the built-in or synced floor. Clear the field to restore automatic mode, which uses the highest manual, synced, or built-in stable version. Valid account device identities still take priority; auth, quota, and input_tokens do not send Version.',
        openaiCodexDefaultValue: 'Baseline when all three global fields are empty: {value}',
        openaiCodexVersionAutoSync: 'Auto-sync official Codex version',
        openaiCodexVersionAutoSyncHint: 'Checks the official repository every 6 hours for stable rust-v releases. Automatic mode updates the global UA engine and Responses/WS Version. Pinned mode updates only the sync record, not the selected version. Manual sync works independently of this toggle and immediately saves the sync record, not other form edits. Valid account device identities keep their local version.',
        openaiCodexOriginatorPresets: 'Choose a common Originator (or type below)',
        openaiCodexUserAgentPresets: 'Choose a UA template using the current Version',
        openaiCodexTemplate: 'Editable template',
        openaiCodexLatestLabel: 'Official latest',
        openaiCodexVersionMode: 'Version mode',
        openaiCodexVersionModeAuto: 'Automatic: highest stable version',
        openaiCodexVersionModePinned: 'Pinned: exact selected version',
        openaiCodexVersionHistory: 'Choose an official historical stable version',
        openaiCodexVersionRefresh: 'Refresh version list',
        openaiCodexVersionLoadMore: 'Load older versions',
        openaiCodexVersionSyncNow: 'Sync official latest version',
        openaiCodexVersionSyncing: 'Syncing…',
        openaiCodexVersionSource: 'Stable releases from:',
        openaiCodexVersionLatest: 'Official latest stable: {version}',
        openaiCodexVersionLoadFailed: 'Failed to load official versions. Retry; existing settings and history are unchanged.',
        openaiCodexVersionSyncFailed: 'Failed to sync the official latest version. Existing version settings are unchanged.',
        openaiCodexVersionSyncSuccess: 'Official stable version synced: {version}. Pinned versions and other unsaved settings are unchanged.',
        openaiCodexVersionSyncedValue: 'Currently synced official stable version: {version}',
      ...
      webSearchEmulation: {
        ...
        enabledHint: 'Global switch. When disabled, web search emulation is inactive for all accounts.',
      ...
      site: {
        ...
        siteNameHint: 'Displayed on the administrator login page and in page titles',
        siteSubtitle: 'Site Subtitle',
        siteSubtitlePlaceholder: 'Claude Code & OpenAI API Gateway',
        siteSubtitleHint: 'Displayed on the administrator login page',
        ...
        customEndpoints: {
          ...
          description: 'Add API endpoint URLs for quick copying on the Global API Keys page',
        ...
        contactInfoHint: 'Customer support contact info displayed in the administrator menu',
      ...
      user_error_view: {
        ...
        description: 'When enabled, users can see their original failed-request details on the usage page. Requires ops monitoring enabled to have data.',
      },
      imageStorage: {
        title: 'Async Image Object Storage',
        description: 'Configure independent S3-compatible object storage for asynchronous OpenAI image tasks. Changes take effect immediately without a restart.',
        enabled: 'Enable Async Image Object Storage',
        enabledHint: 'When enabled, async image results are copied here and returned through the public URL or a presigned URL.',
        endpoint: 'Endpoint',
        region: 'Region',
        bucket: 'Bucket',
        prefix: 'Key prefix',
        ...
        secretConfigured: 'Already configured; leave blank to keep it',
        publicBaseUrl: 'Public base URL',
        publicBaseUrlPlaceholder: 'Leave blank to return presigned URLs',
        presignExpiryHours: 'Presigned URL expiry (hours)',
        maxDownloadBytes: 'Maximum bytes per downloaded result',
        maxDownloadBytesHint: 'Defaults to 33554432 (32 MiB).',
        forcePathStyle: 'Force Path-Style Addressing',
        forcePathStyleHint: 'Use bucket-in-path URLs for S3 services that require them, including some self-hosted object stores.',
        testConnection: 'Test Connection',
        testing: 'Testing...',
        testSuccess: 'Object storage connection succeeded',
        testFailed: 'Object storage connection test failed',
        saved: 'Image storage settings saved',
        loadFailed: 'Failed to load image storage settings',
        saveFailed: 'Failed to save image storage settings'
      },
