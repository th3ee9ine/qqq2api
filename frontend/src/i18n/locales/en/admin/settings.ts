export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexTurnStateHint: "Automatically collect, obtain and renew Turn State per account and actual upstream model. No token entry is needed. The same account and model may reuse it across IPs, for up to one hour.",
        codexTurnStateAutoTitle: "Automatically manage Turn State",
        codexTurnStateAutoHint: "Off by default. Collects tokens from real responses. Matching requests trigger an asynchronous probe when a token is missing or nearing expiry; probes may consume upstream quota. Renewal starts 10 minutes early. A 312 signal immediately revokes the old value and requests a new 292 state, with backoff after failure. Idle accounts are not kept alive. Disabling retains collected values but stops automatic collection, injection and subsequent probes.",
        codexTurnStateDefaultModel: "Default probe model",
        codexTurnStateDefaultModelHint: "Used only when no request model is specified. Acquisition, renewal, 312 recovery and IP pool retries use the model that owns the state; states cannot be shared across models. Enter a full model name, or leave blank to restore gpt-5.5.",
