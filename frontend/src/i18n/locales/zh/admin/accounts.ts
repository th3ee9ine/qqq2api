export default {
    accounts: {
      ...
      dataExported: '数据导出成功',
      dataExportedSkippedShadows: '数据已导出。已跳过 {count} 个 spark 影子账号：其调度配置不在导出文件内，导入后需在重建的影子上重新调优。',
      ...
      dataImportFile: '数据文件',
      dataImportAutoAssignProxy: '为导入账号自动分配空余代理',
      dataImportAutoAssignProxyHint: '忽略文件中的账号代理绑定，按当前使用数从少到多分配给仍有容量的启用代理。',
      ...
      usageWindowsHint: '“5h / 7d”是上游账号（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对账号设定，并非 QQQ2API 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 QQQ2API 端解除该限制。',
      proxyDetailsUnavailable: '代理详情暂不可用',
      upstreamBilling: {
        trustWarning: '此倍率由上游站点针对当前 API Key 自行声明。本网关无法验证该值是否与实际扣费一致；上游站点或中间代理可能返回伪造、过期或被篡改的数据。请结合账单、余额变化和实际用量自行核验。',
      ...
      reAuthorize: '重新授权',
      refreshToken: '刷新令牌',
      refreshSubscription: '刷新订阅到期时间',
      refreshSubscriptionSuccess: 'GPT 订阅到期时间已更新',
      refreshSubscriptionFailed: '获取 GPT 订阅到期时间失败',
      noAccountsYet: '暂无账号',
      ...
      usageWindow: {
        ...
        grokRequests: '请求',
        grokTokens: 'Token',
        grokFreeQuota24hHint: '按 QQQ2API 近 24 小时本地 Token 用量估算（上限 {limit}）',
      ...
      bulkEdit: {
        ...
        manualAssignProxy: '指定同一代理',
        autoAssignProxy: '自动分配代理',
        autoAssignProxyHint: '按代理当前已使用账号数从少到多，将账号分配给仍有容量的启用代理。',
        autoAssignProxyCapacityInsufficient: '启用代理的剩余自动分配容量不足，无法为所有选中账号分配代理。',
      ...
      grok: {
        ...
        ttsTextLabel: 'TTS 文本',
        ttsTextPlaceholder: '例如：Hello from API connectivity test.',
      ...
      anthropic: {
        ...
        apiKeyAuthSchemeDesc: '选择转发到 Anthropic-compatible 上游时使用的 API Key 认证头。',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个 QQQ2API 实例的场景。',
      ...
      proxy: '代理',
      noProxy: '无代理',
      manualAssignProxy: '手动选择代理',
      autoAssignProxy: '自动分配空余代理',
      autoAssignProxyHint: '保存时从仍有容量的启用代理中自动选择，优先使用当前绑定账号较少的代理。',
      autoAssignProxyCapacityInsufficient: '启用代理的剩余容量不足，请增加代理容量或改为手动选择。',
      concurrency: '并发数',
      loadFactor: '负载因子',
      ...
      sessions: {
        action: '已登录设备',
        title: '已登录设备会话',
        description: '查询此 GPT 账号当前已登录的浏览器和 OpenAI 应用会话。设备和位置可能为近似信息。',
        refresh: '刷新',
        selectAll: '全选可退出会话',
        selectedCount: '已选择 {count} 个',
        logoutSelected: '批量退出',
        logoutSelectedTitle: '批量退出设备会话',
        logoutSelectedConfirm: '确定要退出选中的 {count} 个设备会话吗？当前会话不会被退出。',
        logoutSelectedSuccess: '已退出 {count} 个设备会话',
        logoutPartialSuccess: '已退出 {success} 个会话，{failed} 个会话失败，请刷新后重试',
        loading: '正在查询登录会话...',
        retry: '重试',
        empty: '暂无活动会话',
        unknownDevice: '未知设备',
        ...
        lastActiveAt: '最近活动',
        status: '状态',
        statusUnavailable: '会话状态不可用',
        logout: '退出会话',
        logoutTitle: '退出设备会话',
        logoutConfirm: '确定要让“{device}”退出此 GPT 账号吗？',
        logoutSuccess: '设备会话已退出',
        loadFailed: '查询登录会话失败',
        logoutFailed: '退出设备会话失败',
        errors: {
          OPENAI_SESSIONS_PROXY_UNAVAILABLE: '账号代理无法连接 ChatGPT，请检测或更换代理后重试',
          OPENAI_SESSIONS_UPSTREAM_ERROR: 'OpenAI 拒绝了设备会话查询，请刷新账号授权或更换代理后重试',
          OPENAI_SESSIONS_INVALID_RESPONSE: 'OpenAI 返回了暂不支持的设备会话格式',
