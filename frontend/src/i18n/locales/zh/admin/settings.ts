export default {
    settings: {
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 QQQ2API 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 QQQ2API 用户登录',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前 OpenAI Codex 或 Anthropic 用量窗口达到该百分比时，QQQ2API 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      gatewayForwarding: {
        ...
        openaiCodexIdentityHint: '配置 GPT/Codex 调用 OpenAI 上游时的全局客户端身份。内置 Codex Desktop、macOS TUI 和 Ubuntu CLI 三套规范模板，Originator、User-Agent 与 Version 联动保持一致。开启「使用账号本地设备会话身份」且本地身份有效时，账号的三项身份优先。',
        ...
        openaiCodexUserAgentHint: '完整 UA 必须以 client/version 开头。下拉项为三套规范身份模板；选择模板会同步 Originator，选择 Version 会更新 UA engine 及 TUI trailer，Desktop trailer 的应用 build 独立保留。其他客户端身份仍可手动填写。',
        ...
        openaiCodexOriginatorPresets: '选择规范身份（仍可在下方手动输入）',
        openaiCodexUserAgentPresets: '选择规范 UA 模板（与当前 Version 联动）',
        openaiCodexTemplate: '规范模板',
      ...
      payment: {
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 QQQ2API 订单中，上游 type 会作为易支付 type 参数提交。',
