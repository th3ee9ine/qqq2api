export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexTurnStateHint: "按账号和实际上游模型分别采集、获取和续期 Turn State，无需填写 Token。同账号同模型可跨 IP 使用，最长有效一小时。",
        codexTurnStateAutoTitle: "自动管理 Turn State",
        codexTurnStateAutoHint: "默认关闭。开启后从真实响应采集 Token；匹配模型的请求发现 Token 缺失或临近过期时，会异步发起额外探测，可能消耗上游额度。提前 10 分钟续期；收到 312 信号立即停用旧值，重新获取新的 292，失败后退避重试。空闲账号不保活。关闭后保留已采集值，停止自动采集、注入和后续探测。",
        codexTurnStateDefaultModel: "默认探测模型",
        codexTurnStateDefaultModelHint: "仅在没有明确请求模型时使用。获取、续期、312 恢复及 IP 池重试始终使用对应状态的实际模型，不能跨模型复用。可填写完整模型名称，留空恢复 gpt-5.5。",
