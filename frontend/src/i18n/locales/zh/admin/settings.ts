export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiCodexClientVersion: 'Codex 统一版本热修复值',
        openaiCodexClientVersionPlaceholder: '留空跟随官方自动同步',
        openaiCodexClientVersionHint: '通常留空。填写合法稳定版可作为紧急版本下限；User-Agent 首段 engine 与 Responses/WS Version 始终共用手填值、官方同步值和内置下限中的最高版本，不允许旧值降级出站身份。',
        openaiCodexVersionAutoSync: '自动同步 Codex 官方版本',
        openaiCodexVersionAutoSyncHint: '每 6 小时从官方仓库获取最新稳定 rust-v，并同时更新 User-Agent 首段 engine 与 Responses/WS Version。Originator 保持与 UA 客户端名配套；Desktop trailer 的应用 build 独立保留。',
