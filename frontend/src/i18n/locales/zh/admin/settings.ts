export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexTurnStateHint: "按账号和请求模型分别采集、验证和续期 Turn State，无需客户端手动填写。同账号同模型可跨 IP 回放；一小时仅是本地保留上限，不是上游有效期保证。",
        codexTurnStateAutoTitle: "自动管理 Turn State",
        codexTurnStateAutoHint: "默认关闭。候选值只有在 response.created 与 response.completed 的真实模型均匹配，并通过同代理会话与日常代理回放后才替换旧值。State 长度只作诊断；429 会停止换 IP 并按 Retry-After 退避。空闲账号不保活。关闭后保留已验证值，停止自动采集、注入和后续探测。",
        codexTurnStateDefaultModel: "默认探测模型",
        codexTurnStateDefaultModelHint: "仅在没有明确请求模型时使用。获取、续期和代理回放始终使用所属请求模型槽位，不能跨模型复用；codex-auto-review 是独立的合法模型名。可填写完整模型名称，留空恢复 gpt-5.5。",
        ...
        codexTurnStateModelsHint: "逗号分隔；忽略大小写、自动去重，末尾 * 匹配前缀。客户端模型或映射后的上游模型任一命中即可；归一化后最多 1024 字节。",
