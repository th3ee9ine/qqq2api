export default {
    settings: {
      ...
      security: {
        ...
        stepUpHint: '开启后，账号/代理导出、提升管理员等敏感操作需要先完成 TOTP 二次验证（15 分钟内有效）。开启前需本人已启用 2FA；关闭该开关本身也需要二次验证。',
      ...
      panelRateLimit: {
        ...
        proxySafeNote: '登录后的接口按管理员会话维度计数，与来源 IP 无关；公开接口按真实客户端 IP 计数，回环与内网地址（反代内部转发地址）会自动跳过。',
        enabled: '启用面板接口限流',
        enabledHint: '对管理员面板接口按会话主体限流；超出阈值返回 429，窗口重置后自动恢复。',
        userRpm: '管理员请求上限',
        userRpmHint: '管理员每分钟允许的面板 API 请求总数；0 表示不限制。',
        heavyRpm: '管理员重查询上限',
        heavyRpmHint: '管理员每分钟允许的用量/仪表盘等聚合统计查询次数；0 表示不限制。',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前 OpenAI Codex 或 Anthropic 用量窗口达到该百分比时，Sub2API 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
        ...
        accountSchedulingThresholdsRangeHint: '整数 1–100（百分比）。仅 OpenAI 与 Anthropic。'
      },
      ...
      gatewayForwarding: {
        ...
        fingerprintUnificationHint: '统一共享同一 OAuth 账号的客户端请求中的 X-Stainless-* 请求头。关闭后透传客户端原始请求头。',
        ...
        accountLocalDeviceIdentityHint: '开启后，OpenAI Codex 请求优先使用账号本地设备会话的 Originator、User-Agent 和 Version。优先取本地显式 Version，缺失时从本地 UA 首段提取，并保持 UA 首段版本一致；有效本地版本不受全局版本下限影响。本地身份缺失、异常或不匹配时，三项整体回退到下方全局设置及基线身份。',
        openaiCodexIdentityTitle: 'GPT/Codex 上游身份请求头',
        openaiCodexIdentityHint: '配置 GPT/Codex 调用 OpenAI 上游时的全局客户端身份。下方展示的基线默认值仅表示全局三项全部留空时的结果；混合填写时，Originator 和 User-Agent 会联动配对。开启「使用账号本地设备会话身份」且本地身份有效时，账号的 Originator、User-Agent 和 Version 三项优先。',
        codexTurnStateTitle: "Codex Turn State（全局 / 实验性）",
        codexTurnStateHint: "默认关闭。统一覆盖匹配模型的 Codex Responses / Compact / WebSocket 请求头 X-Codex-Turn-State，无需逐账号配置。关闭只停止覆盖，保留已存 token，不改变模型或 reasoning。",
        codexTurnStateNotice: "仅检查公开封装，不验证签名、归属或上游效果。10 块基线和 1 小时参考 TTL 是社区经验；超时、超基线、未来时间戳和未知封装只提示，不停止注入。不保证“不降智”或避免 Overload；自动能力由独立开关控制；开启自动模式后，312 信号会使当前状态提前失效，覆盖上述仅提醒策略。",
        codexTurnStateAutoTitle: "自动采集、生成与续期",
        codexTurnStateAutoHint: "默认关闭，与全局手动覆盖独立。开启后按账号采集真实响应中的 Token；匹配模型的请求发现 Token 缺失或接近 1 小时参考期限时，会异步发送额外 Responses 探测，由上游签发新 Token，可能消耗上游额度。提前 10 分钟续期；收到 312 state（11 个密文块）立即停用旧值并探测新的 292 state（10 个密文块），首次恢复不等待常规 5 分钟冷却，失败后保留退避。旧全局值和原生会话值不能覆盖新状态；空闲账号不保活。关闭后保留已采集值，停止自动采集、注入和后续探测。",
        codexTurnStateToken: "Turn State token",
        codexTurnStateTokenPlaceholder: "粘贴上游返回的 X-Codex-Turn-State",
        codexTurnStateTokenConfiguredPlaceholder: "已配置；留空保留，不回显原值",
        codexTurnStateTokenHint: "仅允许可打印 ASCII，最多 4096 字符。粘贴替换；留空保留。点击“清除”并保存才删除已存值。",
        codexTurnStateClear: "清除",
        codexTurnStateModels: "模型范围",
        codexTurnStateModelsPlaceholder: "例如 gpt-5.5,gpt-5.5-*；留空表示全部",
        codexTurnStateModelsHint: "逗号分隔；忽略大小写、自动去重，末尾 * 匹配前缀。客户端模型或映射后的上游模型任一命中即可；两者均未知时仍允许注入。归一化后最多 1024 字节。",
        codexTurnStateStatusLabel: "手动全局配置诊断",
        codexTurnStatePreview: "待保存 token 的本地预览",
        codexTurnStateVerdict: "封装判定：{verdict}",
        codexTurnStateBlocks: "Fernet 密文块数：{blocks}（参考基线 ≤ 10）",
        codexTurnStateSetAt: "上次设置时间：{value}",
        codexTurnStateIssuedAt: "公开签发时间：{value}",
        codexTurnStateExpiresIn: "距 1 小时参考时效还剩 {duration}（{value}）",
        codexTurnStateExpiredAgo: "已超出参考时效 {duration}（{value}）；仅提醒，仍注入",
        codexTurnStateInjectionActive: "按当前表单保存后，满足模型范围的请求会注入该值；不代表上游已接受。",
        codexTurnStateInjectionInactive: "按当前表单保存后不注入手动全局值：开关关闭、未配置或请求头格式无效；自动账号值仍由上方独立开关控制。",
        codexTurnStateInvalidToken: "Turn State 仅允许可打印 ASCII，长度不得超过 4096。",
        codexTurnStateInvalidModels: "模型范围格式无效：仅支持精确值或末尾 * 前缀匹配，总长度最多 1024 字节，不允许空格或引号。",
        codexTurnStateStatus: {"disabled": "已关闭", "missing": "未配置 token", "invalid": "请求头格式无效", "unknown": "未知封装（仅提示）", "ready": "封装在参考基线内（非效果验证）", "future": "签发时间在未来（仅提示）", "expired": "超过参考 TTL（仅提示）", "suspect": "密文块数超出参考基线（仅提示）"},
        openaiCodexOriginator: 'Originator',
        openaiCodexOriginatorPlaceholder: '全局三项全部留空时使用基线默认值',
        openaiCodexOriginatorHint: '发往 Responses/WS 推理面以及 auth、配额、input_tokens 等凭据/控制面的 Originator。选择预设会同时填入配套 UA；也可在下方自定义。非空值会同步为 UA 首段客户端名。',
        openaiCodexUserAgent: 'User-Agent',
        openaiCodexUserAgentPlaceholder: '全局三项全部留空时使用基线默认值',
        openaiCodexUserAgentHint: '完整 UA 必须以 client/version 开头。下拉项是可编辑模板，并非本机实测指纹；请按需调整系统信息。选择模板会同步 Originator，选择 Version 会更新 UA 首段 engine；Desktop trailer 的应用 build 独立保留。',
        openaiCodexClientVersion: 'Version',
        openaiCodexClientVersionPlaceholder: '全局三项全部留空时使用基线默认值',
        openaiCodexClientVersionHint: '选择历史版本或手动输入会切换到固定模式，保存后精确使用该稳定版本，不受内置/同步版本下限影响；清空恢复自动模式。自动模式取手填值、官方同步值和内置值中的最高稳定版。有效账号本地身份仍优先；auth、配额、input_tokens 不发送 Version。',
        openaiCodexDefaultValue: '全局三项全部留空时的基线默认值：{value}',
        openaiCodexVersionAutoSync: '自动同步 Codex 官方版本',
        openaiCodexVersionAutoSyncHint: '每 6 小时检查官方仓库最新稳定 rust-v。自动模式更新全局 UA engine 与 Responses/WS Version；固定模式仅更新同步记录，不改变所选版本。手动同步按钮不受此开关影响，立即保存同步记录，但不保存其他表单修改。有效账号本地身份仍保留本地版本。',
        openaiCodexOriginatorPresets: '选择常用 Originator（仍可在下方手动输入）',
        openaiCodexUserAgentPresets: '选择常用 UA 模板（与当前 Version 联动）',
        openaiCodexTemplate: '可编辑模板',
        openaiCodexLatestLabel: '官方最新',
        openaiCodexVersionMode: 'Version 生效模式',
        openaiCodexVersionModeAuto: '自动模式：使用最高稳定版',
        openaiCodexVersionModePinned: '固定模式：精确使用所选版本',
        openaiCodexVersionHistory: '选择官方历史稳定版本',
        openaiCodexVersionRefresh: '刷新版本列表',
        openaiCodexVersionLoadMore: '加载更多历史版本',
        openaiCodexVersionSyncNow: '同步官方最新版本',
        openaiCodexVersionSyncing: '正在同步…',
        openaiCodexVersionSource: '稳定版来源：',
        openaiCodexVersionLatest: '官方最新稳定版：{version}',
        openaiCodexVersionLoadFailed: '获取官方版本列表失败，请重试；已有设置和历史列表保持不变。',
        openaiCodexVersionSyncFailed: '同步官方最新版本失败，原有版本设置保持不变。',
        openaiCodexVersionSyncSuccess: '官方稳定版已同步：{version}。固定版本和其他未保存设置保持不变。',
        openaiCodexVersionSyncedValue: '当前同步的官方稳定版：{version}',
      ...
      webSearchEmulation: {
        ...
        enabledHint: '全局开关。关闭后所有账号的 web search 模拟均不生效。',
        providers: '搜索服务商',
        addProvider: '添加服务商',
        providerType: '服务商类型',
        apiKey: 'API Key',
      ...
      site: {
        ...
        description: '自定义站点品牌',
        siteName: '站点名称',
        siteNameHint: '显示在管理员登录页和页面标题中',
        siteNamePlaceholder: 'Sub2API',
        siteSubtitle: '站点副标题',
        siteSubtitleHint: '显示在管理员登录页面',
        siteSubtitlePlaceholder: 'Claude Code 与 OpenAI API 网关',
        apiBaseUrl: 'API 端点地址',
        apiBaseUrlHint: '用于"使用密钥"、"导入到 CC Switch"和回调地址建议，留空则使用当前站点地址',
        apiBaseUrlPlaceholder: 'https://api.example.com',
        tablePreferencesTitle: '通用表格设置',
        tablePreferencesDescription: '设置管理控制台表格组件的默认分页行为',
        ...
        customEndpoints: {
          title: '自定义端点',
          description: '添加额外的 API 端点地址，可在「全局 API 密钥」页面快速复制',
          itemLabel: '端点 #{n}',
          name: '名称',
        ...
        contactInfoHint: '填写客服联系方式，将展示在管理员菜单等位置',
        docUrl: '文档链接',
      ...
      imageStorage: {
        title: '异步生图对象存储',
        description: '为异步 OpenAI 生图任务配置独立的 S3 兼容对象存储。保存后立即生效，无需重启。',
        enabled: '启用异步生图对象存储',
        enabledHint: '开启后，异步生图结果会转存到此对象存储，并通过公开域名或预签名链接返回。',
        endpoint: '端点地址',
        region: '区域',
        bucket: '存储桶',
        prefix: 'Key 前缀',
        ...
        maxDownloadBytes: '单个结果最大下载字节数',
        maxDownloadBytesHint: '默认 33554432（32 MiB）。',
        forcePathStyle: '强制路径风格',
        forcePathStyleHint: '兼容需要 bucket 路径形式的 S3 服务，例如部分自建对象存储。',
        testConnection: '测试连接',
        testing: '测试中...',
        testSuccess: '对象存储连接成功',
        testFailed: '对象存储连接测试失败',
        saved: '图片存储设置已保存',
        loadFailed: '加载图片存储设置失败',
        saveFailed: '保存图片存储设置失败'
      },
      saveSettings: '保存设置',
      saving: '保存中...',
