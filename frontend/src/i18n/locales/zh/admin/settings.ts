export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        codexTurnStateTitle: "Codex Turn State（自动 / 实验性）",
        codexTurnStateHint: "按账号和请求模型分别采集、验证和续期 Turn State，无需客户端手动填写。配置的模型范围会约束自动采集、注入与手动采集；账号可调度状态、代理状态和账号并发配置不作限制。一小时仅是本地保留上限，不是上游有效期保证。",
        codexTurnStateAutoTitle: "自动管理 Turn State",
        codexTurnStateAutoHint: "默认关闭。仅对模型范围内的模型自动采集和注入；范围留空表示全部模型。候选值只有在 response.created 与 response.completed 的真实模型均匹配，并通过采集线路回放后才进入真实用量验证。State 长度只作诊断；429 会按 Retry-After 退避。关闭后保留已验证值，停止自动采集、注入和后续探测；手动采集仍可使用。",
        codexTurnStateProxy: "专属动态代理",
        codexTurnStateProxyHint: "可选。优先使用所选代理进行 Turn State 采集和续期；1024Proxy 动态模板会生成新会话，其他代理直接使用。记录不可用时回退账号代理或直连，不阻断采集。代理密码不会在页面回显。",
        codexTurnStateDefaultModel: "默认探测模型",
        codexTurnStateDefaultModelHint: "仅在请求未明确指定模型时使用。获取、续期和回放仍使用所属请求模型槽位，不能跨模型复用；模型范围非空时，此模型也必须命中范围。codex-auto-review 是独立的合法模型名。可填写完整模型名称，留空恢复 gpt-5.5。",
        codexTurnStateInvalidDefaultModel: "请填写单个模型名称，最多 128 个字符，支持字母、数字及 - _ . : /，不支持空格或通配符。",
        codexTurnStateModels: "模型范围",
        codexTurnStateModelsPlaceholder: "例如 gpt-5.5,gpt-5.5-*；留空表示全部",
        codexTurnStateModelsHint: "自动采集、注入与手动采集仅处理命中范围的模型。逗号分隔；忽略大小写、自动去重，末尾 * 匹配前缀。客户端模型或映射后的上游模型任一命中即可；归一化后最多 1024 字节。",
