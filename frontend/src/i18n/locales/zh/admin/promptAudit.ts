export default {
  promptAudit: {
    ...
    localGuard: {
      title: '越狱防护',
      pageDescription: '查看 API 入口的本地提示注入防护状态与覆盖范围。',
      description: '本地确定性 Guard 在请求进入可选的远程提示词审计前运行，用于拦截高置信度的提示注入与越狱指令。',
      enabled: '已启用',
      disabled: '已关闭',
      unavailable: '状态异常',
      disabledByAdmin: '已由管理员关闭',
      disabledAction: '规则未执行',
      toggle: '启用本地越狱防护',
      switchHint: '关闭后，本地规则不会拦截提示注入；远程提示词审计仍按独立配置运行。',
      saveError: '本地越狱防护开关保存失败，请刷新后重试。',
      execution: '执行方式',
      alwaysOn: '始终启用',
      policy: '策略标识',
      action: '命中动作',
      ...
      behaviors: {
        instructionOverride: '识别“忽略之前指令”等指令覆盖表达。',
    ...
    events: {
      title: '审计事件', description: '按 API Key、分组、入口、风险、Hash 和时间复核事件，详情中可查看完整提示词。', decision: '判定', risk: '风险等级', endpoint: '入口', groupId: '分组 ID', apiKeyId: 'API Key ID', keyword: '关键词',
      ...
      moreConditions: '更多条件（入口 / 关键词 / 分组 / API Key）',
      ...
      selectAll: '选择当前页全部事件', selectEvent: '选择事件 {id}', time: '时间', identity: 'API Key', apiKey: 'API Key 名称', group: '分组', route: '入口 / 模型', result: '判定 / 风险', preview: '脱敏预览', empty: '没有符合条件的事件。',
