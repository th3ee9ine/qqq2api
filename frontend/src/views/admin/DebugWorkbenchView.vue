    ...
    <div class="mx-auto max-w-[1600px] space-y-5">
      ...
      <div class="grid grid-cols-1 gap-5 xl:grid-cols-2">
        ...
        <section class="card overflow-hidden">
          ...
          <div class="space-y-4 p-4" :class="!imageMode && 'opacity-50 pointer-events-none'"><label class="block"><span class="field-label">图像提示词</span><textarea v-model="imageForm.prompt" rows="3" class="input mt-1.5 resize-y" placeholder="描述你想生成的图像…"></textarea></label><div class="grid grid-cols-2 gap-3"><label><span class="field-label">尺寸</span><select v-model="imageForm.size" class="input mt-1.5"><option>1024x1024</option><option>1536x1024</option><option>1024x1536</option></select></label><label><span class="field-label">质量</span><select v-model="imageForm.quality" class="input mt-1.5"><option>standard</option><option>hd</option></select></label></div><div class="flex min-h-24 items-center justify-center rounded-lg border border-dashed border-gray-300 bg-gray-50 text-xs text-gray-400 dark:border-dark-600 dark:bg-dark-800/50">生成结果将在此处预览</div></div>
        </section>
      </div>
...
const currentPayload = computed(() => JSON.stringify(payloads[activeTab.value], null, 2)); const tabDescription = computed(() => tabs.find(t => t.key === activeTab.value)?.label)
