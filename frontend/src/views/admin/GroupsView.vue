        ...
        >
          ...
          <template #cell-actions="{ row }">
            <div class="flex items-center gap-1">
              ...
              <button
                v-if="!authStore.isSimpleMode && isSupportedGroup(row)"
                  ...
                  >
                    ...
                    <td class="px-3 py-2">
                      <div class="flex justify-end gap-1">
                        <button
                          v-if="isSupportedConcretePlatform(route.target_platform)"
                          type="button"
...
const updateEditCodexManifestConfig = (value: CodexModelsManifestConfig) => {
  Object.assign(editCodexManifestConfig, {
    ...
    fallback_to_scheduler: value.fallback_to_scheduler,
  });
};
...
const handleCreateGroup = async () => {
  ...
  try {
    const createGroupForm = { ...createForm };
    // 构建请求数据，包含模型路由配置
    const requestData = {
      ...createGroupForm,
...
const handleEdit = async (group: AdminGroup) => {
  ...
  Object.assign(editCodexManifestConfig, {
    ...
    fallback_to_scheduler:
      savedCodexManifestConfig.fallback_to_scheduler ?? false,
  });
      ...
      codex_models_manifest_config:
        editForm.platform === "openai"
          ? {
              ...
              fallback_to_scheduler:
                editCodexManifestConfig.fallback_to_scheduler,
            }
          : createCodexManifestDefaults(),
      messages_dispatch_model_config:
        editForm.platform === "openai"
...
const handleDuplicate = async (group: AdminGroup) => {
  if (!isSupportedGroup(group) || duplicatingGroupIds.has(group.id)) return;
...
const editCompositeRoute = (route: CompositeModelRoute) => {
  if (!isSupportedConcretePlatform(route.target_platform)) return;
...
const saveCompositeRoute = async () => {
  if (!compositeRoutesGroup.value) return;
  if (!isSupportedConcretePlatform(compositeRouteForm.target_platform)) return;
