...
function errorMessage(value: unknown, duringSend = false): string {
  ...
  // A send may have reached the upstream even when the response was lost.
  // Treat network cancellation as an uncertain outcome only on the send path;
  // eligibility queries remain safe to retry.
