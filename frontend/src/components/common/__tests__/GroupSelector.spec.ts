...
const authState = { isSimpleMode: false, isAccountAdmin: false }
...
const groups = [
  { id: 1, name: 'Basic', platform: 'anthropic', status: 'active', rate_multiplier: 1.2, account_count: 9 },
...
describe('GroupSelector simple-mode binding policy', () => {
  ...
  it('does not expose group billing rates or global account counts to account administrators', () => {
    authState.isAccountAdmin = true
    const wrapper = mountSelector()
    const basic = wrapper.findAll('label').find((label) => label.text().includes('Basic'))

    expect(basic?.attributes('title')).toBe('Basic')
    expect(basic?.text()).not.toContain('9')
  })
})
