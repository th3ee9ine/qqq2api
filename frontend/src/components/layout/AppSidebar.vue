<template>
  ...
  >
        ...
        <template v-for="item in visibleNavItems" :key="item.path">
          ...
          >
            ...
            <span class="sidebar-label sidebar-label-flex" :class="{ 'sidebar-label-collapsed': sidebarCollapsed }">
              ...
              <Icon name="chevronDown" size="sm" class="flex-shrink-0 transition-transform duration-200" :class="isGroupExpanded(item) ? 'rotate-180' : ''" />
            </span>
          </button>
    ...
    <div class="mt-auto border-t border-gray-100 p-3 dark:border-dark-800">
      <button class="sidebar-link mb-2 w-full" :class="{ 'sidebar-link-collapsed': sidebarCollapsed }" :title="sidebarCollapsed ? (isDark ? t('nav.lightMode') : t('nav.darkMode')) : undefined" @click="toggleTheme">
        <Icon v-if="isDark" name="sun" size="md" class="flex-shrink-0 text-amber-500" />
        <Icon v-else name="moon" size="md" class="flex-shrink-0" />
        <span class="sidebar-label" :class="{ 'sidebar-label-collapsed': sidebarCollapsed }">{{ isDark ? t('nav.lightMode') : t('nav.darkMode') }}</span>
      </button>
      <button class="sidebar-link w-full" :class="{ 'sidebar-link-collapsed': sidebarCollapsed }" :title="sidebarCollapsed ? t('nav.expand') : t('nav.collapse')" @click="toggleSidebar">
        <Icon :name="sidebarCollapsed ? 'chevronRight' : 'chevronLeft'" size="md" class="flex-shrink-0" />
        <span class="sidebar-label" :class="{ 'sidebar-label-collapsed': sidebarCollapsed }">{{ t('nav.collapse') }}</span>
      </button>
    </div>
  </aside>
...
const baseNavItems = computed<NavItem[]>(() => [
  ...
  { path: '/admin/groups', label: t('nav.groups'), icon: 'inbox', hideInSimpleMode: true },
...
const visibleNavItems = computed(() => {
  ...
  return filter(baseNavItems.value)
})
...
function handleMenuItemClick(path: string) {
  ...
  const selectors: Record<string, string> = { '/admin/groups': '#sidebar-group-manage', '/admin/accounts': '#sidebar-channel-manage' }
