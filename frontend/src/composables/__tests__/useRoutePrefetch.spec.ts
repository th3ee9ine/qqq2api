...
describe('useRoutePrefetch', () => {
  ...
  describe('_isAdminRoute', () => {
    it('应该正确识别管理员路由', () => {
      ...
      expect(_isAdminRoute('/keys')).toBe(true)
    })

    it('应该正确识别非管理员路由', () => {
      const { _isAdminRoute } = useRoutePrefetch(mockRouter)
      expect(_isAdminRoute('/home')).toBe(false)
  ...
  describe('预加载映射表', () => {
    ...
    it('兼容映射表应该包含管理员路由', () => {
      expect(_userPrefetchMap).toHaveProperty('/admin/dashboard')
      expect(_userPrefetchMap['/admin/dashboard']).toHaveLength(2)
    })
  })
