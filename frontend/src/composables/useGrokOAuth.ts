...
export function useGrokOAuth() {
  ...
  const exchangeAuthCode = async (params: {
    ...
    try {
      ...
      const response = await adminAPI.grok.exchangeCode(payload as any)
      return version === requestVersion ? response : null
    } catch (err: any) {
  ...
  const validateRefreshToken = async (
    ...
    try {
      const response = await adminAPI.grok.refreshGrokToken(refreshToken.trim(), proxyId)
      return version === requestVersion ? response : null
    } catch (err: any) {
      if (version !== requestVersion) return null
      error.value = extractI18nErrorMessage(
  ...
  const validateSSOToken = async (
    ...
    error.value = ''
    try {
      const response = await adminAPI.grok.validateSSOToken(ssoToken.trim(), proxyId)
      return version === requestVersion ? response : null
    } catch (err: any) {
      if (version !== requestVersion) return null
      error.value = extractI18nErrorMessage(
  ...
  const authorizePassword = async (
    ...
    error.value = ''
    try {
      const response = await adminAPI.grok.authorizePassword(emailAndPassword, proxyId)
      return version === requestVersion ? response : null
    } catch (err: any) {
      if (version !== requestVersion) return null
      error.value = extractI18nErrorMessage(
