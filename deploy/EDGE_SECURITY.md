...
QQQ2API supports long-lived SSE and WebSocket requests. Protect the request
...
CIDR/IP addresses that connect directly to QQQ2API. An explicit empty list
...
`gzip_types` and do not use `gzip_types *` for QQQ2API. The
...
QQQ2API `server.trusted_proxies` with the Caddy address/private subnet so the
