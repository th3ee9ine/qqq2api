...
Commands:
  ...
  up [--recreate]       Create and start the complete QQQ2API stack
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another QQQ2API Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value APPLE_CONTAINER_SUB2API_IMAGE qqqrouter1/qqq2api:latest)"
...
create_app_container() {
    info "Creating QQQ2API container..."
    container create \
        --name "${APP_CONTAINER}" \
        --label "${STACK_LABEL_KEY}=${STACK_LABEL_VALUE}" \
        --network "${NETWORK_NAME}" \
...
while true; do
    ...
    set -e
    child_pid=""
    echo "QQQ2API exited with status ${status}; restarting in 1 second..." >&2
    sleep 1
done
' \
        >/dev/null
...
start_app() {
    ...
    if ! wait_for_probe "QQQ2API" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "QQQ2API did not become ready."
    fi
    if ! wait_for_probe "QQQ2API host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "QQQ2API is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "QQQ2API stack stopped; persistent volumes were preserved."
}
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the QQQ2API stack and all persistent data? [y/N] '
    else
        printf 'Delete the QQQ2API containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "QQQ2API stack and persistent data deleted."
    else
        info "QQQ2API stack deleted; persistent volumes were preserved."
    fi
}
