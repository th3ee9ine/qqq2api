...
type UserQuery struct {
	...
	withSuppliedAccounts      *AccountQuery
...
// QuerySuppliedAccounts chains the current query on the "supplied_accounts" edge.
func (_q *UserQuery) QuerySuppliedAccounts() *AccountQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSuppliedAccounts:      _q.withSuppliedAccounts.Clone(),
...
// WithSuppliedAccounts tells the query-builder to eager-load the nodes that are connected to
// the "supplied_accounts" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithSuppliedAccounts(opts ...func(*AccountQuery)) *UserQuery {
	query := (&AccountClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withSuppliedAccounts = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withSuppliedAccounts; query != nil {
		if err := _q.loadSuppliedAccounts(ctx, query, nodes,
			func(n *User) { n.Edges.SuppliedAccounts = []*Account{} },
			func(n *User, e *Account) { n.Edges.SuppliedAccounts = append(n.Edges.SuppliedAccounts, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadSuppliedAccounts(ctx context.Context, query *AccountQuery, nodes []*User, init func(*User), assign func(*User, *Account)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(account.FieldAccountAdminID)
	}
	query.Where(predicate.Account(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SuppliedAccountsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.AccountAdminID
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_admin_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_admin_id" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
