...
// When no OpenAI identity is present, the legacy Claude Code session header and
// Grok conversation header are retained for usage correlation. They are not used
// by Cyber block-key derivation.
//
...
func ExtractOpenAIClientSessionID(c *gin.Context, body []byte) string {
	...
	if !ok {
		if !rejected {
			...
			if c != nil && c.Request != nil && isGrokRequestContext(c) {
...
func openAIIdentityHeader(c *gin.Context, headers []string) (string, bool) {
	...
	for _, header := range headers {
		for _, raw := range c.Request.Header.Values(header) {
