...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	// Simple mode keeps its historical no-wallet/no-subscription behavior. An
	// explicit operator opt-in may still enforce API-key spending windows; in
	// that case persist the usage row and send only the rate-limit counter
	// through the unified billing path below.
