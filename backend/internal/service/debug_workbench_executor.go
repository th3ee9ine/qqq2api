...
func NewDebugWorkbenchService(gateway *OpenAIGatewayService, accounts DebugWorkbenchAccountSource) *DebugWorkbenchService {
	return &DebugWorkbenchService{gateway: gateway, accounts: accounts, slots: make(chan struct{}, 4)}
}
...
func ValidateDebugWorkbenchRequest(input DebugWorkbenchRequest) error {
	...
	switch strings.TrimSpace(input.Session.Action) {
	case "", "new_session":
		return nil
	default:
		return debugInputError(http.StatusBadRequest, "debug requests only support a new independent session")
	}
}

type debugWorkbenchRequestIdentity struct {
	sessionID string
	threadID  string
	turnID    string
	windowID  string
...
func newDebugWorkbenchRequestIdentity() (debugWorkbenchRequestIdentity, error) {
	var ids [4]string
	for i := range ids {
		id, err := uuid.NewRandom()
		if err != nil {
			return debugWorkbenchRequestIdentity{}, fmt.Errorf("generate debug request identity: %w", err)
		}
		ids[i] = id.String()
	}
	return debugWorkbenchRequestIdentity{sessionID: ids[0], threadID: ids[1], turnID: ids[2], windowID: ids[3]}, nil
}

func (i debugWorkbenchRequestIdentity) headers() http.Header {
	headers := make(http.Header)
	headers.Set("session_id", i.sessionID)
	headers.Set("conversation_id", i.sessionID)
	headers.Set("thread-id", i.threadID)
	headers.Set("turn-id", i.turnID)
	headers.Set("x-codex-window-id", i.windowID)
	return headers
}
...
func (s *DebugWorkbenchService) Run(ctx context.Context, ownerID, accountID int64, input DebugWorkbenchRequest) (*DebugWorkbenchResult, error) {
	...
	if s == nil || s.gateway == nil || s.accounts == nil || s.slots == nil {
	...
	identity, err := newDebugWorkbenchRequestIdentity()
	if err != nil {
		return nil, err
	}
	...
	// Snapshot exactly what the editor submitted before account and request rules.
	...
	executionBody, metadataChanged, metadataErr := debugWorkbenchAlignRequestMetadata(req.Header, input.Body, identity)
	if metadataErr != nil {
		return nil, debugInputError(http.StatusBadRequest, metadataErr.Error())
	}
	executionBody, promptCacheKey, cacheNote := debugWorkbenchPromptCache(&account, input.Endpoint, executionBody, identity.sessionID)
	...
	c.Request = req
	// This negative namespace isolates administrator debug traffic from real API
	// keys and is never used for billing.
	...
	result.Warnings = append([]string{
		"调试执行复用正式网关的 HTTP 请求构造与响应转换；本次固定使用 HTTP，不进行 WebSocket 握手。",
		"每次调试都使用全新的独立请求身份；对话历史或 previous_response_id 仅以 Body 中显式提供的内容为准。",
	}, trace.Warnings()...)
	if cacheNote != "" {
		result.Warnings = append(result.Warnings, cacheNote)
	}
	if metadataChanged {
		result.Warnings = append(result.Warnings, "已将 X-Codex-Turn-Metadata 与 Body.client_metadata 中现有的当前会话、线程、回合和窗口标识对齐至本次独立请求身份；父级与子代理上下文保留，入站快照保留原值。")
	}
...
// initial pass excludes transport, credentials and server-generated identities,
// so editor placeholders or an administrator bearer can never become credentials.
...
// that native body field only when absent. Explicit keys are retained.
func debugWorkbenchPromptCache(account *Account, endpoint string, body []byte, sessionID string) ([]byte, string, string) {
	...
	return rebuilt, sessionID, "Body 未指定 prompt_cache_key，已使用本次独立请求的 Session ID 补齐；上游 session_id 继续由正式网关进行账号隔离。"
}
...
func debugWorkbenchAlignRequestMetadata(headers http.Header, body []byte, identity debugWorkbenchRequestIdentity) ([]byte, bool, error) {
	changed := false
	if raw := headers.Get("X-Codex-Turn-Metadata"); raw != "" {
		normalized, edited, err := debugWorkbenchAlignMetadataObject([]byte(raw), identity)
		if err != nil {
	...
	normalized, edited, err := debugWorkbenchAlignMetadataObject(raw, identity)
	if err != nil {
	...
	if embedded, ok := metadata["x-codex-turn-metadata"]; ok {
		...
		embeddedBody, embeddedChanged, err := debugWorkbenchAlignMetadataObject([]byte(encoded), identity)
		if err != nil {
			return nil, false, fmt.Errorf("body.client_metadata.x-codex-turn-metadata must be a JSON object encoded as a string")
		}
		if embeddedChanged {
...
func debugWorkbenchAlignMetadataObject(raw []byte, identity debugWorkbenchRequestIdentity) ([]byte, bool, error) {
	var metadata map[string]json.RawMessage
	if err := json.Unmarshal(raw, &metadata); err != nil || metadata == nil {
		return nil, false, fmt.Errorf("metadata must be a JSON object")
	}
	replacements := map[string]string{
		"session_id": identity.sessionID, "session-id": identity.sessionID, "conversation_id": identity.sessionID,
		"thread_id": identity.threadID, "thread-id": identity.threadID,
		"turn_id": identity.turnID, "turn-id": identity.turnID,
		"window_id": identity.windowID, "x-codex-window-id": identity.windowID,
	}
	changed := false
