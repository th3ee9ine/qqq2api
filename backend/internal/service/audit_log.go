...
import (
	...
	"github.com/th3ee9ine/qqq2api/internal/util/logredact"
)
...
var auditBodySensitiveExactKeys = func() map[string]struct{} {
	builtin := []string{
		...
		// proxy_pool_urls 可包含 protocol://username:password@host:port，
		// custom_key 为用户自设的平台 API Key 明文，
		// session 为 Ollama Cloud 用量的浏览器会话 Cookie 明文。
		"proxy_key", "proxy_pool_urls", "custom_key", "session",
	}
