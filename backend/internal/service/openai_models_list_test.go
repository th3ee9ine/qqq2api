...
func TestPinnedOpenAIModelsListMixedAccountsShareColdCacheAcrossGroups(t *testing.T) {
	...
	// The production OAuth path uses the shared HTTPUpstream transport.  Route
	// the OAuth account to the test server while keeping the API-key account on
	// the deterministic stub so this test exercises both transports together.
	s.httpUpstream = mixedCodexModelsHTTPUpstream{api: s.httpUpstream}
...
type mixedCodexModelsHTTPUpstream struct {
	api HTTPUpstream
}

func (u mixedCodexModelsHTTPUpstream) Do(req *http.Request, proxyURL string, accountID int64, accountConcurrency int) (*http.Response, error) {
	if accountID == 1 {
		return http.DefaultClient.Do(req)
	}
	return u.api.Do(req, proxyURL, accountID, accountConcurrency)
}

func (u mixedCodexModelsHTTPUpstream) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile) (*http.Response, error) {
	if accountID == 1 {
		return http.DefaultClient.Do(req)
	}
	return u.api.DoWithTLS(req, proxyURL, accountID, accountConcurrency, profile)
}
