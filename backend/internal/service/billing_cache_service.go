...
func (s *BillingCacheService) CheckBillingEligibility(ctx context.Context, user *User, apiKey *APIKey, group *Group, subscription *UserSubscription, platform string) error {
	// Simple mode bypasses wallet, subscription, and platform billing by
	// default. Operators may explicitly opt into API-key spending windows.
	...
	// Global API Keys have no user wallet or subscription owner. The admin row
	// is retained only as a technical audit subject. Keep legacy billing checks
	// for non-key internal callers, while key requests are governed by key/group
	// limits below.
	if apiKey == nil {
...
// checkSimpleModeAPIKeyRateLimits deliberately reads the authoritative DB
// counters. Redis updates are asynchronous and therefore cannot be used to
// enforce an explicitly enabled spending window safely.
...
// checkRPM enforces the global group limit. Legacy user ceilings and
// user×group overrides are intentionally ignored because API Keys no longer
// belong to users. Redis failures remain fail-open.
