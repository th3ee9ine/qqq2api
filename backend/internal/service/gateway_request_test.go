...
func TestThinkingEnabledFallbackPreservesQwenOnly(t *testing.T) {
	body := []byte(`{"thinking":{"type":"enabled"}}`)
	for _, model := range []string{"qwen-2-72b-thinking", "qwen3-235b-a22b-thinking-2507", "qwen4-large-thinking"} {
		got := ApplyThinkingEnabledFallback(nil, body, model)
		require.NotNil(t, got, model)
		require.Equal(t, "high", *got, model)
		require.Nil(t, ApplyThinkingEnabledFallback(nil, []byte(`{"thinking":{"type":"disabled"}}`), model), model)
	}
	for _, model := range []string{"kimi-k3", "moonshot-v1", "glm-5.3", "MiniMax-M3", "qwen3-32b", "claude-sonnet-4-5", "gpt-5.5"} {
		require.Nil(t, ApplyThinkingEnabledFallback(nil, body, model), model)
	}
	explicit := "low"
	require.Same(t, &explicit, ApplyThinkingEnabledFallback(&explicit, body, "qwen3-large-thinking"))
}
