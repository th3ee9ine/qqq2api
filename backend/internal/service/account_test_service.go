...
func (s *AccountTestService) BuildOpenAITestDefaults(account *Account, endpoint, prompt string) OpenAITestDefaults {
	...
	case "images/generations":
		...
		if account != nil && account.IsOAuth() {
			parsed := &OpenAIImagesRequest{Endpoint: openAIImagesGenerationsEndpoint, Model: imageModel, Prompt: defaultOpenAIImageTestPrompt, N: 1}
			applyOpenAIImagesDefaults(parsed)
			if raw, targetURL, err := buildOpenAIImagesOAuthPayload(parsed, imageModel); err == nil {
				_ = json.Unmarshal(raw, &result.UpstreamBody)
				result.URL = targetURL
			}
			if usesCodexDirectImages(imageModel) {
				result.Notes = append(result.Notes, "OAuth 图片测试通过原生 Codex Images 通道调用")
			} else {
	...
	if account != nil && account.IsOAuth() {
		result.Notes = append(result.Notes,
			"OAuth 测试与正式网关共用 Codex 能力及路由规则：未声明能力时补 X-Codex-Beta-Features: remote_compaction_v2；X-Codex-Routing-Hint 从最终上游 body 的 model / service_tier 派生，生图按模型使用原生 Codex Images 或 Responses image_generation 通道。",
		)
	}
