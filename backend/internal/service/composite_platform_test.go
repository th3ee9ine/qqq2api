	...
	repo := &compositeOwnershipAccountRepo{
		accounts: []Account{
			...
			},
			{
				...
				Credentials: map[string]any{
					"model_mapping": map[string]any{"reasoning-alias": "claude-sonnet-4-6"},
				},
			},
		},
	}
...
func TestResolveCompositeModelOwnershipRequiresNonEmptyExactMappings(t *testing.T) {
	...
	repo := &compositeOwnershipAccountRepo{
		accounts: []Account{
			...
			},
			{
				...
				Platform: PlatformAnthropic,
				Credentials: map[string]any{
					"model_mapping": map[string]any{"claude-public": "claude-sonnet-4-6"},
				},
			},
		},
	}
	...
	ownership, err := svc.resolveCompositeModelOwnership(context.Background(), groupID, "claude-public")
	require.NoError(t, err)
	require.Equal(t, CompositeModelOwnership{TargetPlatform: PlatformAnthropic, Matched: true}, ownership)
}
...
func TestResolveCompositeModelOwnershipAllowsSamePlatformAndRejectsCrossPlatformAliases(t *testing.T) {
	...
	repo := &compositeOwnershipAccountRepo{
		accounts: []Account{
			...
			{ID: 3, Platform: PlatformAnthropic, Credentials: map[string]any{"model_mapping": map[string]any{"ambiguous": "claude-sonnet-4-6"}}},
		},
	}
	svc := &GatewayService{accountRepo: repo}
