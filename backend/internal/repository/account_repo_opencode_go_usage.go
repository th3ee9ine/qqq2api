...
const (
	...
	opencodeGoUsageMountPlatformsSQL = "'openai', 'anthropic', 'kimi', 'zhipu', 'minimax'"
...
func (r *accountRepository) updateOpenCodeGoUsageGroup(
	...
	apply := func(txCtx context.Context, client *dbent.Client) error {
		...
		if ownerClause, ownerArg, scoped := accountAdminSQLScope(txCtx, "account_admin_id", len(args)+1); scoped {
			query += ownerClause
			args = append(args, ownerArg)
		}
...
func lockOpenCodeGoUsageGroup(
	...
	query := `
		...
		FOR NO KEY UPDATE
	`
	args := []any{apiKey, account.ID, account.Platform, account.Type, string(credentials), proxyID}
	if ownerClause, ownerArg, scoped := accountAdminSQLScope(ctx, "account_admin_id", len(args)+1); scoped {
		query = strings.Replace(query, "\n\t\tORDER BY id\n", ownerClause+"\n\t\tORDER BY id\n", 1)
		args = append(args, ownerArg)
	}
	...
	query := `
		WITH eligible AS (
			...
			WHERE deleted_at IS NULL
				AND status = 'active'
				AND ` + opencodeGoUsageEligibleSQL + `
				AND extra @> '{"opencode_go_usage_auto_refresh": true}'::jsonb` + ownerFilter + `
		), group_activity AS (
			...
			WHERE deleted_at IS NULL
				AND ` + opencodeGoUsageEligibleSQL + `
				AND jsonb_typeof(credentials -> 'api_key') = 'string'` + ownerFilter + `
		...
		LIMIT $4
	`
	args := []any{now.UTC(), debounceSeconds, maxWaitSeconds, limit, minFetchIntervalSeconds}
	if scoped {
		args = append(args, ownerArg)
	}
	rows, err := r.sql.QueryContext(ctx, query, args...)
