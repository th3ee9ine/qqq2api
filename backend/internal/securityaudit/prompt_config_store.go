...
func (m *ConfigManager) buildNextStorage(current storageConfig, req UpdateConfigRequest, actorID int64) (storageConfig, error) {
	...
	next := storageConfig{
		...
		LocalJailbreakGuardEnabled: current.LocalJailbreakGuardEnabled,
	...
	if req.LocalJailbreakGuardEnabled != nil {
		next.LocalJailbreakGuardEnabled = *req.LocalJailbreakGuardEnabled
	}
