...
import (
	...
	"github.com/th3ee9ine/qqq2api/internal/testutil"
...
func TestOpenAIResponsesWebSocketV2PassthroughNonCyberTurnAllowsFollowup(t *testing.T) {
	...
	select {
	case second := <-secondUpstreamFrame:
		// The ingress stamps a per-turn stream start timestamp in
		// client_metadata. Validate the client-visible fields while allowing
		// that transport metadata to evolve.
