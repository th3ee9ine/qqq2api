...
func TestDebugWorkbenchHandlerStrictValidation(t *testing.T) {
	...
	tests := []struct {
		...
		{"removed api key", "7", `{"endpoint":"responses","body":{},"api_key_id":9}`, true, http.StatusBadRequest},
		{"removed verification stage", "7", `{"endpoint":"responses","body":{},"verification_stage":"capture"}`, true, http.StatusBadRequest},
		{"removed session id", "7", `{"endpoint":"responses","body":{},"session":{"id":"legacy","action":"new_session"}}`, true, http.StatusBadRequest},
		{"removed new turn", "7", `{"endpoint":"responses","body":{},"session":{"action":"new_turn"}}`, true, http.StatusBadRequest},
		{"removed continuation", "7", `{"endpoint":"responses","body":{},"session":{"action":"continue_turn"}}`, true, http.StatusBadRequest},
		{"removed replay", "7", `{"endpoint":"responses","body":{},"session":{"action":"replay_capture"}}`, true, http.StatusBadRequest},
