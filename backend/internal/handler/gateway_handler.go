...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if !service.IsActiveAccountPlatform(platform) {
		c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "Platform is not supported"}})
		return
	}
...
func defaultModelIDsForPlatform(platform string) []string {
	...
	case service.PlatformComposite:
		ids := make([]string, 0)
		seen := make(map[string]struct{})
		for _, concretePlatform := range []string{service.PlatformAnthropic, service.PlatformOpenAI} {
...
// Usage returns usage statistics and limits for the authenticated global API Key.
// GET /v1/usage
//
// Two modes:
//   - quota_limited: API Key has quota or rate limits configured. Returns key-level limits/usage.
//   - unrestricted:  No key-level limits. Returns key-level usage only.
...
func (h *GatewayHandler) buildAPIKeyDailyUsage(c *gin.Context, apiKeyID int64, days int) any {
	...
	stats, err := h.usageService.GetAPIKeyDailyUsage(c.Request.Context(), 0, apiKeyID, startTime, endTime)
	if err != nil {
		return nil
	}
	return stats
...
// usageUnrestricted returns only global API Key state. User wallets and
// subscriptions are deliberately absent because keys no longer have owners.
func (h *GatewayHandler) usageUnrestricted(c *gin.Context, apiKey *service.APIKey, usageData gin.H, dailyUsage any, modelStats any) {
	...
	if apiKey.Group != nil {
		resp["group_name"] = apiKey.Group.Name
	}
	if apiKey.ExpiresAt != nil {
		resp["expires_at"] = apiKey.ExpiresAt
		resp["days_until_expiry"] = apiKey.GetDaysUntilExpiry()
