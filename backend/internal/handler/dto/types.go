...
type AdminUser struct {
	...
	// SupplyRateMultiplier is the account administrator's dedicated earnings multiplier.
	SupplyRateMultiplier float64 `json:"supply_rate_multiplier"`
}
...
type APIKey struct {
	...
	CreatedAt   time.Time  `json:"created_at"`
	UpdatedAt   time.Time  `json:"updated_at"`
	// Concurrency is the configured concurrent request limit (0 = unlimited).
	Concurrency int `json:"concurrency"`
