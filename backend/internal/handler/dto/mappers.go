...
func redactAccountManagedExtra(extra map[string]any) map[string]any {
	...
	for key, value := range extra {
		switch key {
		// Keep the response boundary safe for legacy cache entries as well as
		// freshly loaded accounts after these integrations have been removed.
		case "ollama_cloud_usage_session",
...
func AccountListItemFromAccount(a *Account) *AccountListItem {
	...
	return &AccountListItem{
		...
		Credentials: a.Credentials, CredentialsStatus: a.CredentialsStatus, Extra: redactAccountManagedExtra(a.Extra),
